...
#
# Launch docuflow-mcp isolated development container via Apple Container.
#
# Usage:
#   ./scripts/dev-container.sh              # Interactive bash session
#   ./scripts/dev-container.sh -- npm test  # Run a command
#   ./scripts/dev-container.sh -- bash -c "npm run build"
#
# What this does:
#   1. Checks Apple Container system is running (starts it if not)
#   2. Builds docuflow-dev image if missing
...
#
# Security model:
#   The coding agent (claude) cannot read ~/.ssh, ~/.aws, ~/.zshrc,
#   other projects, system files, or anything not explicitly mounted.

set -euo pipefail

IMAGE="docuflow-dev:latest"
CONTAINER_NAME="docuflow-dev"
...
if ! container image list 2>/dev/null | grep -q "docuflow-dev"; then
    log "Building docuflow-dev image (first build: ~5 min, downloads Node + CLIs)..."
    container build \
        --tag "$IMAGE" \
